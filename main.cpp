...
#include "PlayerPower.h" //chỉ số sinh mệnh
...
bool InitData()
{
	...
	g_window = SDL_CreateWindow("Game Cpp SDL 2.0- Blog:nguyenducdat23020039",
...
bool LoadBackground()
{
	bool ret = g_background.LoadImg("anhmap.gif", g_screen);
	if (ret == false)
	    return false;
...
std::vector<ThreatsObject*> MakeThreadList()
{
	...
	for (int i = 0; i < 20; i++)
	{
		...
		if (p_threat != NULL)
		{
			p_threat->LoadImg("quai//threat_left.png", g_screen);
			p_threat->set_clips();
...
int main(int argc, char* argv[])
{
	...
	player_money.SetPos(SCREEN_WIDTH*0.5 - 300, 8);  // vị trí số lượng đồng tiền
	...
	bool mRet = exp_threat.LoadImg ("exp3.png", g_screen);
	exp_main.set_clip();
	if (!mRet) return -1;
